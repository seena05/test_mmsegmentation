...
def save_model(model,
    ...
    if task and ('best_model' in save_name):
        output_model = OutputModel(task=task, framework='pytorch')
        if ema_model:
            output_model.update_weights(weights_filename=(save_path + ".pdema"), target_filename=f'{save_name}.pdema')
        else:
            output_model.update_weights(weights_filename=(save_path + ".pdparams"), target_filename=f'{save_name}.pdparams')
    elif task:
        output_model = OutputModel(task=task, framework='pytorch')
        if ema_model:
            output_model.update_weights(weights_filename=(save_path + ".pdema"), target_filename=f'latest.pdema')
        else:
            output_model.update_weights(weights_filename=(save_path + ".pdparams"), target_filename=f'latest.pdparams')
