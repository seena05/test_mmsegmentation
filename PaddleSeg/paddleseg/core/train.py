                ...
                if clearml_logger:
                    if len(avg_loss_list) > 1:
                        ...
                        for key, value in avg_loss_dict.items():
                            clearml_logger.report_scalar('train', key, iteration=iter, value=value)
                    else:
                        clearml_logger.report_scalar('train', 'loss', iteration=iter, value=avg_loss)
                    clearml_logger.report_scalar('train', 'batch_cost', iteration=iter, value=avg_train_batch_cost)
                    clearml_logger.report_scalar('train', 'reader_cost', iteration=iter, value=avg_train_reader_cost)
                if use_vdl:
                ...
                if val_dataset is not None:
                    if mean_iou > best_mean_iou:
                        ...
                        if task:
                            ...
                            output_model.update_weights(weights_filename=os.path.join(best_model_dir, 'model.pdparams'), target_filename=f'best_model.pdparams', iteration=best_model_iter)
                    logger.info(
                    ...
                    clearml_logger = Logger.current_logger()
                    if clearml_logger:
                        clearml_logger.report_scalar('val', 'mIoU', iteration=iter, value=mean_iou)
                        clearml_logger.report_scalar('val', 'Acc', iteration=iter, value=acc)
                    if use_vdl:
